                       ...
                       const AVCodecTag *tags, int for_asf, int ignore_extradata)
{
    ...
    raw_pal_avi = !for_asf && par->codec_id == AV_CODEC_ID_RAWVIDEO &&
                  !par->codec_tag &&
            par->bits_per_coded_sample >= 1 && par->bits_per_coded_sample <= 8;
    if (!par->extradata_size && raw_pal_avi)
        extradata_size = 4 * (1 << par->bits_per_coded_sample);
